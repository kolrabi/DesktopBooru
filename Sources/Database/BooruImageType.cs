...
namespace Booru
{
	...
	{
		private static byte[] magic = new byte[16];
		...
		{		
			try {
				lock(magic) {
					...
					if (magic[0] == 0x47 && magic[1] == 0x49 && magic[2] == 0x46) {
						...
						try {
							...
							BooruApp.BooruApplication.Log.Log(BooruLog.Category.Files, BooruLog.Severity.Warning, path+" is not a valid gif: "+ex.Message);
						}
						return BooruImageType.Image;
					}
				...
				BooruApp.BooruApplication.Log.Log(BooruLog.Category.Files, BooruLog.Severity.Info, "Unknown file type: "+path+": "+ex.Message);
				callBooru = false;
				return BooruImageType.Unknown;
			}
		}
